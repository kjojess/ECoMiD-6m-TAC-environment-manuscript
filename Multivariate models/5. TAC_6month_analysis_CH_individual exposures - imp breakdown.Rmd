...
## Linear regression models - floors, water, toilet
```{r}
models <- d 

#select exposures of interest
exposures <- c("comm_type", "rainy", "toilet_type", "water_cait")

#, "toilet_type", "floors", "water_cait"
...
#select outcomes
 outcomes <- c("anyinfection", "coinfection", "numpathogens", "anyvirus", 
               ...
               "Adenovirus", #45
               #"Cryptosporidium", #6
               "Norovirus", #20 - floors didn't converge
               "Salmonella", #25 
               "Campylobacter", #38
               "EAEC", #171
               "DAEC", #85
               #"tEPEC", #21 - water, toilet didn't converge
               "aEPEC", #78
               "ETEC") #47
               #"EIEC_or_Shigella") #14 - water, toilet, floors didn't converge
               #"EHEC") #5
               #"STEC") #2
...
for(i in outcomes){
  ...
  for(j in exposures){
    ...
    if(j %in% c("comm_type", "toilet_type")){
      res <- broom::tidy(model, conf.int = T, exponentiate = T)[c(2:4),c(1,2,5:7)]
      } else if(j %in% c("floors")){
        res <- broom::tidy(model, conf.int = T, exponentiate = T)[c(2:5),c(1,2,5:7)]
      } else if(j %in% c("water_cait")){
        res <- broom::tidy(model, conf.int = T, exponentiate = T)[c(2:6),c(1,2,5:7)]
      } else if(j %in% c("birthorder", "hfiacat")){
...
saveRDS(results.out, "Export_files/MVM results_individual exposures_breakdown_toilet_water_20240614.rds") 

saveRDS(vif, "Export_files/MVM results_vif_individual exposures_breakdown_toilet_water_20240614.rds") 
...
saveRDS(covars_out, "Export_files/MVM covariates used_individual exposures_breakdown_toilet_water_20240614.rds") 
...
save_as_docx(restable, path = "Figures/MVM results_individual exposures_breakdown_toilet_water_20240614.docx")
