...
#Linear regression models - "imp_water_jmp",
```{r}
models <- d 

#select exposures of interest
exposures <- c("imp_water_jmp")
...
results_adjusted <- results%>%
  ...
  select(outcome, exposure, p.value, adjusted)

results_imp_water_jmp <- results_adjusted%>%
  mutate(FDR_p.value = p.adjust(p.value, "fdr"))

#merge
results <- results%>%
  left_join(results_imp_water_jmp, by=c("outcome", "exposure", "p.value", "adjusted"))
...
saveRDS(results.out, "Export_files/MVM results_individual exposures_jmp_20240614.rds") 

saveRDS(vif, "Export_files/MVM results_vif_individual exposures_jmp_20240614.rds") 
...
saveRDS(covars_out, "Export_files/MVM covariates used_individual exposures_jmp_20240614.rds") 
...
save_as_docx(restable, path = "Figures/MVM results_individual exposures_jmp_20240614.docx")
